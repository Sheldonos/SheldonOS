---
name: skill-creator
description: Guide for creating or updating skills that extend Sheldon via specialized knowledge, workflows, or tool integrations. For any modification or improvement request, MUST first read this skill and follow its update workflow instead of editing files directly.
...
Skills are modular, self-contained packages that extend Sheldon's capabilities by providing specialized knowledge, workflows, and tools. Think of them as "onboarding guides" for specific domains or tasks—they transform Sheldon from a general-purpose agent into a specialized agent equipped with procedural knowledge that no model can fully possess.
...
The context window is a public good. Skills share the context window with everything else Sheldon needs: system prompt, conversation history, other Skills' metadata, and the actual user request.

**Default assumption: Sheldon is already very smart.** Only add context Sheldon doesn't already have. Challenge each piece of information: "Does Sheldon really need this explanation?" and "Does this paragraph justify its token cost?"
...
Think of Sheldon as exploring a path: a narrow bridge with cliffs needs specific guardrails (low freedom), while an open field allows many routes (high freedom).
...
- **Frontmatter** (YAML): Contains `name` and `description` fields. These are the only fields that Sheldon reads to determine when the skill gets used, thus it is very important to be clear and comprehensive in describing what the skill is, and when it should be used.
...
Sheldon only loads the relevant reference file when needed.
...
When editing the (newly-generated or existing) skill, remember that the skill is being created for another instance of Sheldon to use. Include information that would be beneficial and non-obvious to Sheldon. Consider what procedural knowledge, domain-specific details, or reusable assets would help another Sheldon instance execute these tasks more effectively.
